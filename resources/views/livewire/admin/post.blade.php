<div class="relative" x-data="{ isDragging: false, uploadProgress: 0 }" x-on:livewire-upload-start="uploadProgress = 0"
    x-on:livewire-upload-progress="uploadProgress = $event.detail.progress"
    x-on:livewire-upload-finish="uploadProgress = 100; setTimeout(() => uploadProgress = 0, 1000)">
    {{-- Error Message --}}
    @if ($uploadError)
    ...
    <div @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false"
        ...
        <input type="file" multiple x-ref="fileInput" wire:model="songFiles" class="hidden" accept=".mp3,.wav">

        <div class="flex flex-col md:flex-row items-center justify-center gap-4">
            <div class="text-center md:text-left">
                <p class="text-white font-medium text-lg">Drag your songs here</p>
                <p class="text-gray-500 text-sm">.mp3 or .wav, max. 100MB</p>
            </div>
    ...
    {{-- ==================== UPLOAD PROGRESS BAR ==================== --}}
    <div x-show="uploadProgress > 0" x-cloak class="mt-4">
        ...
        <div class="bg-[#1a1a1a] rounded-xl overflow-hidden" x-data="{ expanded: true }">
            ...
            <div class="flex items-center justify-between p-4 border-b border-gray-800">
                ...
                <div class="flex items-center gap-4">
                    ...
                    {{-- Actions --}}
                    <button wire:click="removeSong({{ $index }})"
            ...
            <div x-show="expanded" x-collapse class="p-4">
                <div class="flex gap-6">
                    ...
                    <div class="flex-shrink-0">
                        <div
                            ...
                            <label for="cover-{{ $index }}"
                                class="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-xs font-medium py-2 text-center cursor-pointer hover:bg-black/80 transition">
                                {{ isset($coverFiles[$index]) ? 'CHANGE' : 'ADD COVER' }}
                            </label>
                            <input type="file" id="cover-{{ $index }}" class="hidden"
                                wire:model="coverFiles.{{ $index }}" accept="image/*">
                        </div>
                        {{-- Loading indicator for cover --}}
                        <div wire:loading wire:target="coverFiles.{{ $index }}" class="mt-2 text-center">
                    ...
                    <div class="flex-1 space-y-3">
                        ...
                        {{-- Tags --}}
                        <input type="text" placeholder="Add tags" wire:model.defer="uploadedSongs.{{ $index }}.tags"
                            class="w-full bg-[#2a2a2a] text-white placeholder-gray-500 rounded-lg border-0 px-4 py-3 focus:ring-2 focus:ring-green-500">
                    </div>
                </div>
            </div>
    ...
    <div class="mt-6 flex flex-col gap-4">
        ...
        <div class="flex justify-end">
            ...
            <button wire:click="saveAllSongs" wire:loading.attr="disabled" wire:target="saveAllSongs"
