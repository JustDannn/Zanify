...
@if($isLikedSongs)
<a href="{{ route('liked-songs') }}" wire:navigate @elseif($isRecentlyPlayed) <a href="{{ route('recently-played') }}"
    wire:navigate @elseif($isAlbum && $cardId) <a href="{{ route('album', $cardId) }}" wire:navigate @elseif($isArtist
    && $cardId) <a href="{{ route('artist', $cardId) }}" wire:navigate @else <div @endif
    ...
    {{-- Image/Icon Section --}}
    @if ($isLikedSongs)
    <div class="w-20 h-20 bg-linear-to-br from-[#450af5] to-[#c4efd9] flex items-center justify-center shrink-0">
    ...
    <div class="w-20 h-20 bg-linear-to-br from-[#1db954] to-[#191414] flex items-center justify-center shrink-0">
        <svg class="w-8 h-8 text-white" fill="currentColor" viewBox="0 0 24 24">
            <path
